...
export default function StatsCard({ label, value, suffix, color, index }) {
  ...
  return (
    <motion.div
      ...
      className={`flex-1 min-w-[220px] bg-neutral-900 border-2 rounded-xl p-8 text-center shadow-lg border-transparent bg-clip-padding relative before:absolute before:inset-0 before:rounded-xl before:bg-gradient-to-br before:${color} before:opacity-60 before:-z-10`}
    >
      <motion.span
      ...
      <div className="mt-2 text-gray-300 text-lg font-medium">{label}</div>
    </motion.div>
  );
} 