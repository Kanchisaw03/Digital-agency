...
export default function Footer() {
  return (
    <footer className="bg-dark border-t border-neutral-800 py-10 mt-10">
      <div className="container mx-auto flex flex-col md:flex-row items-center justify-between gap-6 px-4">
        ...
        <nav className="flex gap-6 flex-wrap justify-center">
          {navLinks.map(link => (
            <Link key={link.to} to={link.to} className="text-gray-300 hover:text-primary-blue font-medium transition">
              {link.name}
            </Link>
          ))}
        </nav>
        <div className="flex gap-4">
          {socials.map(s => (
            <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" aria-label={s.label} className="text-xl text-gray-400 hover:text-primary-blue transition">
              {s.icon}
            </a>
          ))}
        </div>
